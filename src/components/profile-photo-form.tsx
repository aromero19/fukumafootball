...
export default function ProfilePhotoForm({ entryId, photoUrl, admin = false }: { entryId: number; photoUrl: string | null; admin?: boolean }) {
  ...
  return <div className="form-stack"><ImageUpload profile destination={`kind=${admin ? 'admin-profile' : 'profile'}&entry=${entryId}`} onSaved={setUrl} onBusyChange={setUploading} /><fieldset disabled={uploading}><AdminForm action={admin ? savePlayerPhoto : updateProfilePhoto} className="form-stack">
    ...
    <label>Profile image URL<input name="photo_url" type="url" maxLength={2048} value={url} onChange={event => setUrl(event.target.value)} placeholder="https://example.com/my-photo.jpg" aria-describedby={`photo-help-${entryId}`} /></label>
    <p id={`photo-help-${entryId}`} className="muted">Paste a publicly accessible HTTPS image link. Square photos work best. Missing or unavailable photos display a gray silhouette. Leave the field blank and save to remove your photo.</p>
