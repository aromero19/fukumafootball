...
export function HistoryChart({points,title}:{points:Point[];title:string}) {
 ...
 return <section className="card history-chart"><h2>{title}</h2><p className="muted">Pick accuracy for games with published results. Select a point to see the saved picks. Missing periods appear as gaps.</p>
  ...
  <div className="history-plot"><svg viewBox="0 0 740 270" role="group" aria-label={title}>
   ...
   {points.map((p,i)=><g key={p.label}>
    <a href={p.href} aria-label={`${p.label}: ${p.correct} correct picks, ${(100*p.correct/p.scored).toFixed(1)} percent. View picks.`} onFocus={()=>setFocused(i)} onBlur={()=>setFocused(null)} onMouseEnter={()=>setFocused(i)} onMouseLeave={()=>setFocused(null)}>
     <circle cx={x(p)} cy={y(p)} r="10" fill="transparent"/>
     <circle cx={x(p)} cy={y(p)} r={focused===i?7:5} fill="var(--blue)" stroke="white" strokeWidth="2"/>
     <title>{p.label}: {p.correct} correct picks ({(100*p.correct/p.scored).toFixed(1)}%)</title>
    </a>
    {(points.length<=10||i%2===0||i===points.length-1)&&<text x={x(p)} y="248" textAnchor="middle" fontSize="12" fill="currentColor">{p.label}</text>}
   </g>)}
  </svg></div>
  <p className="history-chart-detail" aria-live="polite">{selected?`${selected.label}: ${selected.correct} correct picks (${(100*selected.correct/selected.scored).toFixed(1)}%)`:"Hover or focus a point for details. The same figures are available in the table below."}</p>
 </section>;
}
