...
type Props = { seasonActive: boolean; images: { team_id: number; theme_id: number; image_url: string; thumbnail_url: string | null; active: boolean }[]; defaultThemeId: number | null; year: number; week: number; games: Game[]; teams: Team[]; themes: Theme[]; picks: Pick[]; themeId: number | null; initialEntryId: number };

export default function PicksForm({ seasonActive, images, defaultThemeId, year, week, games, teams, themes, picks, themeId, initialEntryId }: Props) {
  ...
  async function submit() {
    if (busy.current || !seasonActive || !games.length) return;
    if (!entry || !theme) return setMessage("Choose a theme first.");
