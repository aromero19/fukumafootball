...
type QueueRow = { request_id:string;year:number;week:number;created_at:Date;sent_at:Date|null;attempts:number;failed:boolean;name_first:string;name_last:string };
...
export default async function AdminEmail() {
  ...
  try {
    snapshot = await operationsDatabase(actorId, async client => {
      ...
      const counts = await client.query("select count(*) filter(where sent_at is null)::integer as pending,count(*) filter(where sent_at is not null)::integer as sent,count(*) filter(where sent_at is null and attempts>0)::integer as attempted from private.email_outbox");
      const queue = await client.query("select o.request_id,o.year,o.week,o.created_at,o.sent_at,o.attempts,(o.last_error is not null) as failed,e.name_first,e.name_last from private.email_outbox o join public.entry e using(entry_id) order by (o.sent_at is null) desc,o.created_at desc limit 50");
