...
export default async function AdminEmail() {
  ...
  if (!configured) return <section className="card"><h2>Email operations setup</h2><p>The operator must configure the server-only operations connection before this page can load private email settings. Delivery also requires the production email provider configuration.</p></section>;
  let snapshot;
  try {
  ...
  return <div className="form-stack"><section className="card"><h2>Confirmation email settings</h2><p>Use a sender verified with the email provider. Saved picks remain valid even when delivery is disabled or fails.</p>
    <AdminForm action={saveEmailSettings} className="form-stack">
      ...
      <label className="check-label"><input name="enabled" type="checkbox" defaultChecked={settings.enabled} /> Enable confirmation emails</label>
    ...
    <section className="card"><h2>Delivery queue</h2><p>{snapshot.counts.pending} pending · {snapshot.counts.attempted} pending with prior attempts · {snapshot.counts.sent} delivered</p><p className="muted">Latest 50 messages, pending first. Automatic delivery tries at most five times and stops retrying messages more than 23 hours old after their first attempt. New submissions and confirmed retries attempt delivery automatically in production. Failed or interrupted delivery stays queued for review; the operator can run the worker to recover pending messages. A worker failure after provider acceptance can leave delivery uncertain.</p>
