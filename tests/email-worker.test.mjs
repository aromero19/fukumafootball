...
function mock({enabled=true,locked=true,rows=[row]}={}) {
 const queries=[];
 return {queries, async query(sql,params){queries.push({sql,params});
  ...
  if(sql.startsWith("select o.request_id"))return {rows};
...
test("a missing contact skips delivery without a failed or uncertain attempt",async()=>{
 const client=mock({rows:[{...row,email:null}]});
 const result=await processEmailOutbox(client,()=>assert.fail('must not send'));
 assert.equal(result.attempted,0);assert.equal(result.failed,0);assert.equal(result.sent,0);
 assert.ok(client.queries.some(q=>q.sql.includes('set skipped_at=')));
 assert.ok(!client.queries.some(q=>q.sql.includes('set attempts=')));
});
